...
const NftCard = ({nft, onBuy}) => {
  return (
    <div className='flex flex-col items-center h-[340px] overflow-hidden rounded-lg shadow-md p-4 group'>
      ...
      <Image src={nft.image} alt='nft.name'  className='w-full h-60 overflow-hidden object-cover rounded-lg' />
       <div className='absolute bottom-0 flex flex-col items-center bg-gray-500 w-full py-4 z-10 rounded-b-lg transition-transform duration-300 group-hover:translate-y-20 translate-y-34'>
       <h3 className='text-lg font-semibold text-gray-200 text-center'>{nft.name}</h3>
        ...
        <p className='mt-2 font-bold text-center'>{nft.price}</p>
